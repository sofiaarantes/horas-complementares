            ...
            <div id="content">
                ...
                <div class="container mt-5">
                    ...
                    <div class="text-center mb-4">
                        <h2 class="font-weight-bold text-primary">Painel do Admin</h2>
                    </div>
                    <br>
                    <a href="{{ route('ppc') }}" class="btn btn-primary">
                        <i class="fas fa-upload mr-2"></i> Criar novo ppc
                    </a>
                    <br><br>
                    <!-- Lista de ppcs existentes -->
                    @foreach($ppcs as $ppc)
                    <div class="card shadow mb-3">
                        <div class="card-body">
                            <h5><strong>PPC:</strong></h5>
                            <strong>Ano:</strong> {{ $ppc->ano }}
                            <br><strong>Criado em:</strong> {{ $ppc->created_at->format('d/m/Y') }}
                            <br><strong>Última atualização em:</strong> {{ $ppc->updated_at->format('d/m/Y') }}
                            ...
                            <div class="d-flex justify-content-between">
                                ...
                                <div>
                                    <form action="{{ route('duplicarPpc', $ppc->id) }}" method="POST" style="display:inline;">
                                        @csrf
                                        <button type="submit" class="btn btn-success btn-sm mr-2">
                                            <i class="fas fa-check-circle mr-1"></i> Duplicar
                                        </button>
                                    </form>
                                    <a href="{{ route('editarPpc', $ppc->id) }}" class="btn btn-info btn-sm">
                                        <i class="fas fa-pencil mr-1"></i> Editar
                                    </a>
                                    <form action="{{ route('excluirPpc', $ppc->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Tem certeza que deseja excluir este Ppc?');">
                                        @csrf
                                        @method('DELETE')
